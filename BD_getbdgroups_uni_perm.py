...
parser.add_argument('--ff', help='Fertile Focal List File Name (use full path if not in cwd)', type=str, required=True)
...
with open(args.ff) as list_fertile_focals:
	fertile_focal_ids_tmp = [line.rstrip() for line in list_fertile_focals]
with open(args.f) as list_fertile_nonfocals:
	fertile_nonfocal_ids = [line.rstrip() for line in list_fertile_nonfocals]
list_steriles.close() #sanity
list_fertile_focals.close()
list_fertile_nonfocals.close()
...
if '' in fertile_focal_ids_tmp:
	while('' in fertile_focal_ids_tmp):
		fertile_ids_tmp.remove('')
if '' in fertile_nonfocal_ids_tmp:
	while('' in fertile_nonfocal_ids_tmp):
		fertile_nonfocal_ids_tmp.remove('')

## Shuffle phenotype identity
sterile_len = len(sterile_ids_tmp)
grouped_list = sterile_ids_tmp + fertile_focal_ids_tmp
grouped_list = rd.shuffle(grouped_list)
sterile_ids = grouped_list[0:sterile_len]
fertile_ids_group_tmp = grouped_list[sterile_len:]
